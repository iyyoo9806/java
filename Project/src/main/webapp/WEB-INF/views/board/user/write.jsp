			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div class="panel-body">
						<form class="form-horizontal" method="post" action="writing" enctype="multipart/form-data" onsubmit="return checkFileSize();">
...
function check(){
    ...
    // CKEditor 인스턴스 가져오기
    var editor = CKEDITOR.instances.contentText;
...
<script>
    function checkFileSize() {
        ...
        var maxSize = 524288000; // 최대 허용 파일 크기 (약 500MB)

        for (var i = 0; i < files.length; i++) {
            if (files[i].files.length > 0) {
                var fileSize = files[i].files[0].size; // 파일 크기 확인
                if (fileSize > maxSize) {
                    alert("파일 크기가 제한을 초과했습니다. 최대 크기는 500MB입니다.");
                    return false; // 업로드 취소
                }
            }
        }
        return true; // 파일 크기가 모두 허용 범위 내일 경우 업로드 진행
    }
</script>
